...
def send_reset_email_sync(user_email: str, otp: str):
    smtp_server, smtp_port, smtp_user, smtp_password = _get_smtp_credentials()

    if not smtp_user or not smtp_password:
        print("--------------------------------------------------")
        print("WARNING: Missing SMTP credentials.")
        print(f"MOCK RESET EMAIL → {user_email}")
        print(f"RESET OTP CODE: {otp}")
        print("--------------------------------------------------")
        return

    msg = EmailMessage()
    msg['Subject']    = "Cognate — Cryptographic Key Reset Requested"
    msg['From']       = formataddr(('Cognate Workspace', smtp_user))
    msg['Reply-To']   = smtp_user
    msg['To']         = user_email
    msg['Message-ID'] = make_msgid(domain='cognate.internal')
    ...
    plain_text = f"A password reset was requested for your Cognate workspace. Use this one-time code to authorize a new credential: {otp}. This code expires in 10 minutes. Do not share it with anyone."
    msg.set_content(plain_text)
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:500px;background-color:#ffffff;border-radius:24px;border:1px solid #e5e7eb;padding:40px;text-align:center;">
                        <tr>
                            <td>
                                ...
                                <p style="margin:0 0 24px;font-size:15px;color:#6b7280;line-height:1.6;">
                                    A password reset was requested for your Cognate workspace.<br>
                                    Use the one-time code below to authorize a new credential.
                                </p>
                                ...
                                <!-- Warning Banner -->
                                <div style="background-color:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:16px;margin:0 0 24px;">
                                    <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#dc2626;">
                                        &#9888; Security Warning
                                    </p>
                                    <p style="margin:0;font-size:13px;color:#9ca3af;line-height:1.5;">
                                        This code expires in <strong>10 minutes</strong>.<br>
                                        <span style="color:#ef4444;font-weight:600;">Do not share it with anyone.</span>
                                    </p>
                                </div>

                                <hr style="border:none;border-top:1px solid #e5e7eb;margin:0 0 24px;">
                                <p style="margin:0;font-size:12px;color:#9ca3af;">
                                    If you did not request this reset, your account may be at risk. Please secure it immediately.
                                </p>
                            </td>
                        </tr>
                    </table>
